...
fetch('field_GEOJSON/demo_CornShow.geojson')
  .then(res => res.json())
  .then(data => {
...
// Load SoybeanShowDate1; - assigned to a variable
const SoybeanShowDate1 = fetch('field_rasters/SoybeanShowcase/c629NDVI_demo_Soybean_sc.tif') //when saveing from ArcGIS Pro save it outside of GDB and add .tif at the end
  .then(response => response.arrayBuffer())
  .then(arrayBuffer => parseGeoraster(arrayBuffer))
  .then(georaster => {
    rasterLayers['SoybeanShowDate1'] = new GeoRasterLayer({
      georaster: georaster,
      opacity: 0.8,
      resolution: 128,
  ...
  .catch(err => console.log('SoybeanShowDate1 error:', err));

// Load SoybeanShowDate2 - assigned to a variable
const SoybeanShowDate2 = fetch('field_rasters/SoybeanShowcase/c79NDVI_demo_Soybean_sc.tif') //when saveing from ArcGIS Pro save it outside of GDB and add .tif at the end
  .then(response => response.arrayBuffer())
  .then(arrayBuffer => parseGeoraster(arrayBuffer))
  .then(georaster => {
    rasterLayers['SoybeanShowDate2'] = new GeoRasterLayer({
      georaster: georaster,
      opacity: 0.8,
      resolution: 128,
      pixelValuesToColorFn: function(values) {
  ...
  .catch(err => console.log('SoybeanShowDate2 error:', err));

Promise.all([SoybeanShowDate1, SoybeanShowDate2]).then(() => {
  rasterLayers['SoybeanShowDate1'].addTo(map);
  rasterLayers['SoybeanShowDate2'].addTo(map);

  rasterLayers['SoybeanShowDate2'].getContainer().style.display = 'none';

 

  document.getElementById('toggleSoybeanShowDate1-desktop').checked = true;
  document.getElementById('toggleSoybeanShowDate1-mobile').checked = true;
