...
const CornShowDate1 = fetch('field_rasters/CornShowcase/c629NDVI_demo_corn_sc.tif') //when saveing from ArcGIS Pro save it outside of GDB and add .tif at the end
  ...
  .then(georaster => {
    rasterLayers['CornShowDate1'] = new GeoRasterLayer({
      georaster: georaster,
      opacity: 0.8,
      resolution: 128,
...
const CornShowDate2 = fetch('field_rasters/CornShowcase/c79NDVI_demo_corn_sc.tif') //when saveing from ArcGIS Pro save it outside of GDB and add .tif at the end
  ...
  .then(arrayBuffer => parseGeoraster(arrayBuffer))
  .then(georaster => {
    rasterLayers['CornShowDate2'] = new GeoRasterLayer({
      georaster: georaster,
      opacity: 0.8,
      resolution: 128,
      pixelValuesToColorFn: function(values) {
  ...
  .catch(err => console.log('CornShowDate2 error:', err));

Promise.all([CornShowDate1, CornShowDate2]).then(() => {
  rasterLayers['CornShowDate1'].addTo(map);
  rasterLayers['CornShowDate2'].addTo(map);

  rasterLayers['CornShowDate2'].getContainer().style.display = 'none';

 

  document.getElementById('toggleCornShowDate1-desktop').checked = true;
  document.getElementById('toggleCornShowDate1-mobile').checked = true;
