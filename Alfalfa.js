...
fetch('field_GEOJSON/demo_Alfalfa.geojson')
  .then(res => res.json())
  .then(data => {
...
// Load AlfalfaDate1 - assigned to a variable
const AlfalfaDate1 = fetch('field_rasters/LakeVer/6152026/Demo/Alfalfa/61526_Alfalfa.tif') //when saveing from ArcGIS Pro save it outside of GDB and add .tif at the end
  .then(response => response.arrayBuffer())
  .then(arrayBuffer => parseGeoraster(arrayBuffer))
  .then(georaster => {
    rasterLayers['AlfalfaDate1'] = new GeoRasterLayer({
      georaster: georaster,
      opacity: 0.8,
      resolution: 128,
  ...
  .catch(err => console.log('AlfalfaDate1 error:', err));

// Load AlfalfaDate2 - assigned to a variable
const AlfalfaDate2 = fetch('field_rasters/LakeVer/622026/Demo/Alfalfa/6226_Alfalfa.TIF') //when saveing from ArcGIS Pro save it outside of GDB and add .tif at the end
  .then(response => response.arrayBuffer())
  .then(arrayBuffer => parseGeoraster(arrayBuffer))
  .then(georaster => {
    rasterLayers['AlfalfaDate2'] = new GeoRasterLayer({
      georaster: georaster,
      opacity: 0.8,
      resolution: 128,
      pixelValuesToColorFn: function(values) {
  ...
  .catch(err => console.log('AlfalfaDate2 error:', err));

Promise.all([AlfalfaDate1, AlfalfaDate2]).then(() => {
  rasterLayers['AlfalfaDate1'].addTo(map);
  rasterLayers['AlfalfaDate2'].addTo(map);

  rasterLayers['AlfalfaDate2'].getContainer().style.display = 'none';

 

  document.getElementById('toggleAlfalfaDate1-desktop').checked = true;
  document.getElementById('toggleAlfalfaDate1-mobile').checked = true;
