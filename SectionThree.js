...
const plotCharacteristics = {
  ...
  806 : { Treatment: "Liberty Ultra, Enlist One, Cornerstone 5 Plus, Class Act NG, + Strikelock",
  ...
  805: { Treatment: "Liberty Ultra, Enlist One, Cornerstone 5 Plus, + Class Act NG",
         "Liberty Ultra Rate": "24 oz/ac", "Enlist One Rate": "16 oz/ac", 
...
function buildPlotPopup(props, custom) {
  let html = `<div class="popup-title">Experiment ${props.PLOT ?? props.PageName}</div>`;
...
fetch('field_GEOJSON/Section_Three_exp.geojson')
  .then(res => res.json())
  .then(data => {
...
// Load SectionThreeDate1 - assigned to a variable
const SectionThreeDate1 = fetch('field_rasters/GrassExp/working/629NDVI_Grass_exp.tif') //when saveing from ArcGIS Pro save it outside of GDB and add .tif at the end
  .then(response => response.arrayBuffer())
  .then(arrayBuffer => parseGeoraster(arrayBuffer))
  .then(georaster => {
    rasterLayers['SectionThreeDate1'] = new GeoRasterLayer({
      georaster: georaster,
      opacity: 0.8,
      resolution: 128,
  ...
  .catch(err => console.log('SectionThreeDate1 error:', err));

// Load SectionThreeDate2 - assigned to a variable
const SectionThreeDate2 = fetch('field_rasters/GrassExp/working/79NDVI_Grass_exp.tif') //when saveing from ArcGIS Pro save it outside of GDB and add .tif at the end
  .then(response => response.arrayBuffer())
  .then(arrayBuffer => parseGeoraster(arrayBuffer))
  .then(georaster => {
    rasterLayers['SectionThreeDate2'] = new GeoRasterLayer({
      georaster: georaster,
      opacity: 0.8,
      resolution: 128,
      pixelValuesToColorFn: function(values) {
  ...
  .catch(err => console.log('SectionThreeDate2 error:', err));

Promise.all([SectionThreeDate1, SectionThreeDate2]).then(() => {
  rasterLayers['SectionThreeDate1'].addTo(map);
  rasterLayers['SectionThreeDate2'].addTo(map);

  rasterLayers['SectionThreeDate2'].getContainer().style.display = 'none';

 

  document.getElementById('toggleSectionThreeDate1-desktop').checked = true;
  document.getElementById('toggleSectionThreeDate1-mobile').checked = true;
